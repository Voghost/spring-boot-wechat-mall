...
Page({
  ...
  async getFloorListData(){
    ...
    // const new_floor=floor.map(item=>({...item,navigator_url:item.navigator_url.replace('?','/index?')}))
    const new_floor=floor.map(
      item=>({...item,navigator_url:item.floor_title.navigator_url.replace('?','/index?')})
            )
